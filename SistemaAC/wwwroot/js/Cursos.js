...
class Cursos {
    ...
    getCategorias(id, funcion) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                document.getElementById('CategoriaCursos').options[0] = new Option("Seleccion un curso", 0);
                if (0 < response.length) {
                    for (var i = 0; i < response.length; i++) {
                        if (0 === funcion) {
                            //para pasar valores al select del modal de Cursos
                ...
                if (this.creditos === "") {
                    ...
                    if (this.horas === "") {
                        ...
                        if (this.costos === "") {
                            ...
                            if (this.categoria === "0") {
                                document.getElementById("mensaje").innerHTML = "Seleciona un curso";
                            } else {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: action,
                                    data: { id, nombre, descripcion, creditos, horas, costo, estado, categoria, action, funcion },
                                    success: (response) => {
            ...
            success: (response) => {
                ...
                if (funcion === 0) {
                    if (response[0].estado) {
                        document.getElementById("titleCursos").innerHTML = "Esta seguro de desactivar el curso " + response[0].nombre;
                    } else {
                        document.getElementById("titleCursos").innerHTML = "Esta seguro de habilitar el curso " + response[0].nombre;
                    }
    ...
    editarEstadoCurso(id, funcion) {
        ...
        var action = this.action;
        //se recuperan los datos almacenados en el objeto de tipo promise 'then'
        promesa.then(data => {
            //id = data.id;
            nombre = data.nombre
