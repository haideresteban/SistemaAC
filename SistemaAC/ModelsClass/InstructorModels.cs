...
namespace SistemaAC.ModelsClass
{
    ...
    public class InstructorModels
    {
        ...
        internal List<object[]> filtrarInstructores(int numPagina, string valor, string order)
        {
            ...
            {
                query = instructores.Where(p => p.Nombres.StartsWith(valor)).Skip(inicio).Take(reg_por_pagina);
            }
            cant = query.Count();
            foreach (var item in query)
            {
                ...
                dataFilter += "<tr>" +
                   ...
                   "<td>" + Estado + " </td>" +
                   "<td>" +
                   "<a data-toggle='modal' data-target='#modalAS' onclick='editarInstructor(" + item.ID + ',' + 1 + ")'  class='btn btn-success'>Edit</a>" +
                   "</td>" +
                   "<td>" +
                   "<a data-toggle='modal' data-target='#modalDeleteAS' onclick='deleteInstructor(" + item.ID + ")'  class='btn btn-danger'>Delete</a>" +
                   "</td>" +
               "</tr>";
            }
            if (valor == "null")
