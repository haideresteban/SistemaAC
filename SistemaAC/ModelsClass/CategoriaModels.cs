...
namespace SistemaAC.ModelsClass
{
    public class CategoriaModels
    {
        ...
        public List<object[]> filtarDatos(int numPagina, string valor, string order)
        {
            int count = 0, cant, numeroRegistros = 0, inicio = 0, reg_por_pagina = 2;
            ...
            if ((numeroRegistros % reg_por_pagina) > 0)
            {
                numeroRegistros += 1;
            }
            inicio = (numPagina - 1) * reg_por_pagina;
            can_paginas = (numeroRegistros / reg_por_pagina);
            if (valor == null)
            {
                //omitir primers registros de acuerdo a la variable reg_por_pagina 
                //take retorna datos desde el inicio  de acuerdo a la variable almacenado reg_por_pagina
                query = categorias.Skip(inicio).Take(reg_por_pagina);
            }
            else
            {
                //nombres que coinciden en valor, el Where StartsWith
